    ...
    tiles (scale: 2) {
        multiAttributeTile(name:"windowShade", type: "generic", width: 3, height: 4){
            ...
            tileAttribute ("device.position", key: "SLIDER_CONTROL") {
                attributeState "position", action:"setLevel"
            }
        }
...
def open() { 
	logging("${device.displayName} - Executing Open","info") 
	setFibaro(99,99)     
}

def close() { 
	logging("${device.displayName} - Executing Close","info") 
	setFibaro(0,0) 
}
...
def correctLevel(Integer value) {
...
def setLevel(valuex) {
	def value = correctLevel(valuex.toInteger())
	logging("${device.displayName} - Executing Set Position $value","info") 
...
def parseFibaro(map) {
    ...
    /*switch (val1) {
   	...
   	logging("${device.displayName} - Opened Value $val1")
	if (val1 >= (openOffset ?: 95)) {
    	sendEvent(name: "windowShade", value: "open");
    }
    else if  (val1 <= (closeOffset ?: 5)) {
    	sendEvent(name: "windowShade", value: "closed");
    }else{
    	sendEvent(name: "windowShade", value: "partially open");
    }
